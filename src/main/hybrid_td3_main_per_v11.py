...
import src.models.v11_Hybrid_TD3_model_PER as td3_model
...
def generate_preds(model_dict, features, actions, prob_weights, c_actions,
    ...
    return_ctrs = torch.zeros(size=(features.size()[0], len(model_dict))).to(device)
    ...
    for i in choose_model_lens:  # 根据ddqn_model的action,判断要选择ensemble的数量
        ...
        if i == pretrain_model_len:
            ...
            current_ctrs_temp = torch.zeros(size=[len(with_action_indexs), len(model_dict)]).to(device)
            ...
            current_c_actions_temp = torch.zeros(size=[len(with_action_indexs), len(model_dict)]).to(device)
            current_ctrs_temp = torch.zeros(size=[len(with_action_indexs), len(model_dict)]).to(device)
            current_softmax_weights_temp = torch.zeros(size=[len(with_action_indexs), len(model_dict)]).to(device)
...
def test(rl_model, model_dict, embedding_layer, data_loader, device):
    ...
    with torch.no_grad():
        for i, (features, labels) in enumerate(data_loader):
            ...
            y, rewards, return_c_actions, return_ctrs = generate_preds(model_dict, features, actions, ensemble_c_actions, c_actions,
...
if __name__ == '__main__':
    ...
    #gap = int(round((args.run_steps // args.rl_iter_size) // args.record_times, -1))  # 打印间隔
    #print(gap)
    # gap = 5000
    ...
    batch_gen = get_list_data(train_data, args.rl_iter_size, True)# 要不要早停
    while intime_steps <= args.stop_steps:
        batchs = batch_gen.__next__()
    ...
    '''
    if is_early_stop:
        test_rl_model = get_model(model_dict_len, feature_nums, field_nums, args.latent_dims, args.init_lr_a, args.init_lr_c, data_len,
                     args.rl_batch_size,
                     args.memory_size, args.seed, device, campaign_id)
    ...
    test_predicts, test_auc, test_actions, test_prob_weights = submission(test_rl_model, model_dict,
