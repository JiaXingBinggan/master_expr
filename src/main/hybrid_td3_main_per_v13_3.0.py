...
def generate_preds(thres, ensemble_nums, pretrain_y_preds, actions, prob_weights, c_actions,
                   labels, device, mode):
    ...
    return_c_actions = torch.zeros_like(pretrain_y_preds).float()
    ...
    for i in range(1, ensemble_nums + 1):
        ...
        current_return_c_action_with_clks = torch.zeros(size=[len(with_clk_indexs), ensemble_nums]).to(device)
        current_return_c_action_without_clks = torch.zeros(size=[len(without_clk_indexs), ensemble_nums]).to(device)
        current_return_c_actions = torch.zeros(size=[len(with_action_indexs), ensemble_nums]).to(device)

        current_return_prob_with_clks = torch.zeros(size=[len(with_clk_indexs), ensemble_nums]).to(device)
        ...
        current_return_c_actions[with_clk_indexs, :] = current_return_c_action_with_clks
        current_return_c_actions[without_clk_indexs, :] = current_return_c_action_without_clks
    ...
    return model_y_preds, basic_rewards, return_c_actions, return_ctrs

def test(rl_model, ensemble_nums, thres, data_loader, device):
    ...
    with torch.no_grad():
        for i, (current_pretrain_y_preds, labels) in enumerate(data_loader):
            ...
            y, rewards, return_c_actions, return_ctrs = generate_preds(thres, ensemble_nums, current_pretrain_y_preds, actions, ensemble_c_actions, c_actions,
...
def submission(rl_model, ensemble_nums, thres, data_loader, device):
    targets, predicts = list(), list()
    final_actions = torch.LongTensor().to(device)
    final_prob_weights = torch.FloatTensor().to(device)
    with torch.no_grad():
        ...
        for seed in seeds:
            ...
            #     current_pretrain_y_preds, labels = pretrain_y_preds.float().to(device), labels.long().unsqueeze(1).to(device)
                ...
                y_preds, rewards, return_c_actions, return_ctrs = \
                    generate_preds(thres, args.ensemble_nums, current_pretrain_y_preds, ensemble_d_actions, ensemble_c_actions, c_actions, labels, device,
                                   mode='train')
                ...
                if intime_steps % gap == 0:
                    test_auc, predicts, test_rewards, actions, prob_weights = test(rl_model, args.ensemble_nums, thres,
