...
def get_model(action_nums, feature_nums, field_nums, latent_dims, init_lr_a, init_lr_c, data_len, train_batch_size,
              memory_size, seed, device, campaign_id):
    RL_model = td3_model.Hybrid_TD3_Model(feature_nums, field_nums, latent_dims,
                                          ...
                                          memory_size=memory_size, random_seed=seed, device=device)
    return RL_model
...
def generate_preds(model_dict, features, actions, prob_weights, c_actions,
    ...
    for i in choose_model_lens:  # 根据ddqn_model的action,判断要选择ensemble的数量
        ...
        if i == pretrain_model_len:
            ...
            current_weights = sort_prob_weights[with_action_indexs][:, :i] * -1
            ...
            for m in range(i):
                ...
                for k in range(pretrain_model_len):
                    ...
                    current_c_actions_temp[choose_model_indexs, k] = current_weights[choose_model_indexs, m]
    ...
    return y_preds, rewards, return_c_actions # c_actions原思路是只返回对应d_action维的连续值,相当于一个mask掩码的概念
...
def eva_stopping(valid_rewards, type, args):  # early stopping
    if len(valid_rewards) >= args.early_stop_iter:
        ...
        reward_div_mean = sum([abs(valid_rewards[-i] - valid_rewards[-i - 1]) for i in
...
if __name__ == '__main__':
    ...
    for key in indexs_dict:
        ...
        for i in range(len(ensemble_model_list)):
            current_model = get_ensemble_model(ensemble_model_list[i], feature_nums, field_nums, args.latent_dims).to(
                device)
            pretrain_params = torch.load(
                args.save_param_dir + args.campaign_id + ensemble_model_list[i] + str(key) + 'best.pth')
        ...
        train_iters = len(k_train_data) // args.rl_iter_size
        gap = int(round(train_iters // args.record_times, -1))  # 打印间隔

        train_dataset = Data.libsvm_dataset(k_train_data[:, 1:], k_train_data[:, 0])
        val_dataset = Data.libsvm_dataset(k_val_data[:, 1:], k_val_data[:, 0])

        train_data_loader = torch.utils.data.DataLoader(train_dataset, batch_size=args.rl_iter_size, num_workers=8)
        val_data_loader = torch.utils.data.DataLoader(val_dataset, batch_size=args.rl_gen_batch_size, num_workers=8)

        data_len = len(k_train_data)

        rl_model = get_model(model_dict_len, feature_nums, field_nums, args.latent_dims, args.init_lr_a, args.init_lr_c,
                             data_len,
                             args.rl_batch_size,
        ...
        FM_pretrain_params = torch.load(args.save_param_dir + args.campaign_id + 'FM' + str(key) + 'best.pth')
        ...
        is_val = False
        # for i, batchs in enumerate(get_list_data(k_train_data, args.rl_batch_size, True)):
        #     features, labels = torch.Tensor(batchs[:, 1:]).long().to(device), torch.unsqueeze(
        #         torch.LongTensor(batchs[:, 0]), 1).to(device)
        for i, (features, labels) in enumerate(train_data_loader):
            ...
            transitions = torch.cat(
                [features.float(), return_c_actions, d_q_values, ensemble_d_actions.float(), rewards],
                dim=1)
            ...
            if i % gap == 0:
                ...
                logger.info('Model {}, Fold {}, timesteps {}, val auc {}, val rewards {}, [{}s]'.format(
                    args.rl_model_name, key, i * args.rl_iter_size, auc, test_rewards,
                ...
                rl_model.temprature = max(rl_model.temprature_min,
                                      rl_model.temprature_max -
                                          (rl_model.temprature_max - rl_model.temprature_min) * i / train_iters)

                torch.cuda.empty_cache()

            if (i * args.rl_iter_size) > args.rl_batch_size:
        ...
        test_predicts, test_auc, test_actions, test_prob_weights = submission(rl_model, model_dict,
                                                                              ...
                                                                              device)
        logger.info('Model {}, Fold {}, test auc {}, [{}s]'.format(args.rl_model_name, key,
                                                                   test_auc,
        ...
        prob_weights_df.to_csv(submission_path + 'test_prob_weights' + str(key) + '.csv', header=None)

        actions_df = pd.DataFrame(data=test_actions)
        actions_df.to_csv(submission_path + 'test_actions' + str(key) + '.csv', header=None)

        valid_aucs_df = pd.DataFrame(data=val_aucs)
        valid_aucs_df.to_csv(submission_path + 'val_aucs' + str(key) + '.csv', header=None)
        ...
        val_rewards_records_df.to_csv(submission_path + 'val_reward_records' + str(key) + '.csv', index=None)

        train_critics_df = pd.DataFrame(data=train_critics)
        train_critics_df.to_csv(submission_path + 'train_critics' + str(key) + '.csv', header=None)
