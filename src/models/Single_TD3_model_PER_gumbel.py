...
class TD3_Model():
    ...
    def choose_action(self, state):
        ...
        with torch.no_grad():
            ...
            res_actions = gumbel_softmax_sample(logits=action_means,
                                         temprature=self.temprature, hard=False)

        return res_actions, torch.argmax(res_actions, dim=-1) + 1
    ...
    def choose_best_action(self, state):
        ...
        with torch.no_grad():
            ...
            res_actions = gumbel_softmax_sample(logits=action_means,
                                            temprature=self.temprature_eva, hard=False)
    ...
    def learn(self):
        ...
        with torch.no_grad():
            ...
            actions_means_next = gumbel_softmax_sample(logits=actions_means_next,
                                                   temprature=self.temprature, hard=False)
        ...
        if self.learn_iter % self.policy_freq == 0:
            ...
            d_actions_q_values_ = gumbel_softmax_sample(logits=actions_means, temprature=self.temprature_eva,
                                                        hard=False)

            critic_value = self.Critic.evaluate_q_1(b_s, d_actions_q_values_)
