...
import src.models.v2_Hybrid_SAC_model as sac_model
...
def main(data_path, dataset_name, campaign_id, latent_dims, model_name,
    ...
    for epoch_i in range(epoch):
        ...
        for i, (features, labels) in enumerate(tqdm.tqdm(train_data_loader, smoothing=0, mininterval=1.0)):
            ...
            if (i + 1) >= 10:
                c_critic_loss, d_critic_loss = rl_model.learn(embedding_layer)
                train_c_critics.append(c_critic_loss)
                train_d_critics.append(d_critic_loss)
    ...
    train_c_critics_df = pd.DataFrame(data=train_c_critics)
    train_c_critics_df.to_csv(submission_path + 'train_c_critics.csv', header=None)

    train_d_critics_df = pd.DataFrame(data=train_d_critics)
    train_d_critics_df.to_csv(submission_path + 'train_d_critics.csv', header=None)
