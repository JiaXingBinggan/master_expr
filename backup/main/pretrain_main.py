...
def get_dataset(datapath, dataset_name, campaign_id, valid_day, test_day):
    ...
    # train_log = pd.read_csv(data_path + 'train.csv', header=None)
    # columns = train_log.iloc[0, :].values
    # train_log = train_log.drop([0]).values
    # valid_log = train_log[valid_index_start: valid_index_end, :]
    # valid_log_df = pd.DataFrame(data=valid_log, columns=columns)
    # valid_log_df.to_csv(data_path + str(valid_day) + '_log.csv', index=None)
    ...
    # test_log = train_log[test_index_start: test_index_end, :]
    # test_log_df = pd.DataFrame(data=test_log, columns=columns)
    # test_log_df.to_csv(data_path + str(test_day) + '_log.csv', index=None)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', default='FM', help='LR, FM, FFM, W&D, FNN, DeepFM, IPNN, OPNN, DCN, AFM')
