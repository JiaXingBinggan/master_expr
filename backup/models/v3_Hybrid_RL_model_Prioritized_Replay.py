...
class Hybrid_RL_Model():
    ...
    def choose_action(self, state, exploration_rate):
        ...
        random_c_actions = torch.clamp(c_actions + torch.normal(torch.zeros(size=[len(state), self.c_action_nums]), exploration_rate).to(self.device), -1, 1)
        ensemble_c_actions = torch.softmax(random_c_actions, dim=-1)

        random_d_q_values = torch.softmax(d_q_values + torch.normal(torch.zeros(size=[len(state), self.d_action_nums]), exploration_rate).to(self.device), dim=-1)
