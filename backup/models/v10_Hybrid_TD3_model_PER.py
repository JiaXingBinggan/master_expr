...
class Hybrid_Actor(nn.Module):
    def __init__(self, input_dims, action_nums):
        ...
        self.c_action_layer = nn.Sequential(
            nn.Linear(neuron_nums[2], self.action_dims),
            nn.Tanh()
        )
...
class Hybrid_TD3_Model():
    ...
    def to_next_state_c_actions(self, next_d_actions, next_c_actions):
        ...
        for i in range(sortindex_c_actions.size()[1]):
            ...
            for m in range(sortindex_c_actions.size()[1]):
                ...
                return_c_actions_temp[with_choose_index, m:m+1] = current_next_c_actions_temp + torch.clamp(torch.randn_like(current_next_c_actions_temp) * 0.2, -0.5, 0.5)
    ...
    def learn(self, embedding_layer):
        ...
        nn.utils.clip_grad_norm_(self.Hybrid_Critic.parameters(), max_norm=100)
        ...
        if self.learn_iter % self.policy_freq == 0:
            ...
            c_reg = 0.5 * (c_actions_means ** 2).mean()
            d_reg = 0.5 * (d_actions_q_values ** 2).mean()
            ...
            c_a_loss.backward()
            nn.utils.clip_grad_norm_(self.Hybrid_Actor.parameters(), max_norm=100)
