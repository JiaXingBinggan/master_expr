...
class RLB_DP_I:
	...
	def run(self, budget, budget_para, auction_in, bid_log_path, N, c0, max_bid, input_type="file reader", delimiter=" ", save_log=False):
		auction = 0
		imp = 0
		clk = 0
		cost = 0
		...
		hours = []
		thetas = []
		for i, line in enumerate(auction_in):
			if i == 0:
				continue
			count += 1

			if cost >= budget: # 5000000, 30297100, 30231716:
				break
